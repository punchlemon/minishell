...
/*   Updated: 2024/11/24 15:07:10 by hnakayam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	store_cmds(t_cmd_a *cmd, const t_tkn *src_tkns, size_t *s_i)
{
	...
	while (type_is_cmd(src_tkns[*s_i + c_len].type))
		c_len++;
	cmd->tkns = ft_xcalloc(sizeof(t_tkn) * (c_len + 1));
	if (!cmd->tkns)
		return (0);
...
static int	store_cond(t_cond *cond, const t_tkn *src_tkns, size_t *s_i)
{
	...
	while (type_is_cmds(src_tkns[*s_i + ++tmp].type))
		if (src_tkns[*s_i + tmp].type == PIPE)
			p_len++;
	cond->cmds = ft_xcalloc(sizeof(t_tkn *) * (p_len + 1));
	if (!cond->cmds)
...
t_cond	*parser(const t_tkn *src_tkns)
{
	...
	while (src_tkns[++tmp].type != TAIL)
		if (type_is_and_or(src_tkns[tmp].type))
			c_len++;
	s_i = 0;
	conds = ft_xcalloc(sizeof(t_cond) * (c_len + 1));
	if (!conds)
		return (NULL);
	conds[0].type = HEAD;
