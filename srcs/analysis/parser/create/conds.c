...
/*   Updated: 2024/11/02 23:17:16 by retanaka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	store_conds(t_cond *conds, const t_tkn *tkns,
	const size_t t_end)
{
	size_t	t_i;
	size_t	tmp;
	...
	while (t_i < t_end)
	{
		...
		conds[c_i].cmds = create_cmds(&tkns[t_i], tmp);
		if (!conds[c_i].cmds)
...
t_cond	*create_conds(const t_tkn *tkns, size_t t_end)
{
	t_cond	*conds;
	size_t	c_len;
	...
	if (!conds)
		return (NULL);
	conds->type = HEAD;
	if (!store_conds(conds, tkns, t_end))
		return (NULL);
	return (conds);
}
